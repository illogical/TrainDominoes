...
namespace Assets.Scripts.Helpers
{
    public static class PositionHelper
    {
        ...
        // UNTESTED
        public static float GetCameraDistanceByWidth(Camera camera, float totalWidth)
        {
            ...
            //float distance = cameraDistance * totalWidth / cameraView; // Combined wanted distance from the object

            //distance += 0.5f * totalWidth; // Estimated offset from the center to the outside of the object
            //camera.transform.position = bounds.center - distance * camera.transform.forward;

            var fov = camera.fieldOfView * (Math.PI / 180);
            var cameraZ = Math.Abs(totalWidth / 4 * Math.Tan(fov * 2));

            return (float)cameraZ;
        }

        // UNTESTED
        public static float GetWidthForLineOfObjects(float startPos, float endPos, float objectWidth, float sideMargin)
        {
            var totalWidth = Math.Abs(endPos - startPos) + objectWidth + sideMargin;
            return totalWidth;
        }
    }
}
